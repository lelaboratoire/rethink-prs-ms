...
Measuring averaged allelic effects across all genomics backgrounds and environmental exposures, GWAS have primarly sought to discern genetic association with phenotypes of interest by studying single nucleotide polymorphisms (SNPs) and other DNA variants across the human genome [@iFUfVw9V; @5cdeEdUS; @12kQ0EOWQ].
...
Importantly, the PRS modeling method presented above assumes a simplified genetic architecture consisting of independent $\beta_j$ weights.                                             
However, many such modeling challenges exist with respect to understanding interactive relationships among genes and profiling patient susceptibility to non-Mendelian disorders.
...
The Multifactor Dimensionality Reduction (MDR) method is one such nonparametric, model-free framework that addresses these challenges and has been extensively applied to detect nonlinear complex GxG interactions associated with individual disease [@E26QhGxD; @1BqLrlGsj]. 
...
Through the following sections, we briefly review the features of the MDR and MB-MDR software, describe how our new MRS method evaluates polygenic risk, and test MRS profiling performance to the standard PRS method on evidence-based simulated dataset collections generated from the evolutionary-based Heuristic Identification of Biological Architectures for simulating Complex Hierarchical Interactions (HIBACHI) method.
